...
public class AuthService {
    ...
    private LoginTokens login(LoginRequest request, UserRole requestedRole) {
        User user = userRepository.findByLoginId(request.loginId().trim())
                .orElseThrow(() -> new BusinessException(HttpStatus.UNAUTHORIZED, "INVALID_LOGIN", "사용자 ID 또는 비밀번호가 올바르지 않습니다."));

        if (!passwordEncoder.matches(request.password(), user.getPassword())) {
            throw new BusinessException(HttpStatus.UNAUTHORIZED, "INVALID_LOGIN", "사용자 ID 또는 비밀번호가 올바르지 않습니다.");
        }
